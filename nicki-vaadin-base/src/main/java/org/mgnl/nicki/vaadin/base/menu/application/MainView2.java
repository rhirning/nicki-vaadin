...
import org.mgnl.nicki.vaadin.base.menu.navigation.TabNavigation;
...
@Slf4j
public class MainView2 extends AppLayout implements RouterLayout, NavigationMainView {
...
//	private TableNavigation navigation;
	...
	public MainView2(Person user, String configPath) throws IllegalAccessException, InvocationTargetException, InstantiationException {
		this.user = user;
		...
		this.navigation = new TabNavigation(this);
		...
		if (config.containsKey(TITLE)) {
	        final Label title = new Label(config.get(TITLE));
	        top.add(title);
		}
        addToNavbar(top);
	...
	private void logout() {
        // TODO AccessControlFactory.getInstance().createAccessControl().signOut();
    }
...
/*
    ...
    private Component createFolder(String caption, Icon icon) {
        final Label label = new Label();
        label.setClassName("menu-folder");
        label.add(icon);
        label.add(new Span(caption));
        icon.setSize("24px");
        return label;
    }
  */  
